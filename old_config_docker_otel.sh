...
# 3. Injeção da configuração do receiver (Idempotente)
if ! grep -q "^  docker_stats:" "$CONFIG_FILE"; then
    ...
    # b) Adiciona o novo receiver na pipeline de métricas
    sed -i '/^ *metrics:/,/receivers: \[/ s/receivers: \[/receivers: \[docker_stats, /' "$CONFIG_FILE"
